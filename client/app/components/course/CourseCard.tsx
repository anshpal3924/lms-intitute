...
const CourseCard: React.FC<CourseProps> = ({ course, isProfile }) => {
  return (
    <Link href={isProfile ? `/course-access/${course._id}` : `/course/${course._id}`}>
      <article className="bg-white dark:bg-gray-800 group relative rounded-lg overflow-hidden shadow-lg hover:shadow-2xl transform duration-200 cursor-pointer">
        ...
        <div className="px-3 py-4">
          ...
          <h3 className="text-sm text-gray-500 pb-2">
            <span className="bg-green-600 py-1 px-2 text-white rounded-lg">
              {course.level}
            </span>
          </h3>
